...
- **Localhost vs Netlify** :
 /cart and /checkout routes were protected by server-side middleware (proxy.ts) that checked for a cookie  but  auth lived in localStorage + Redux (client-side). On Netlify, the middleware ran before React loaded, saw no cookie, and redirected  to /login even though user was already logged in. This caused an infinite redirect loop in production but worked fine on localhost.
Solution
